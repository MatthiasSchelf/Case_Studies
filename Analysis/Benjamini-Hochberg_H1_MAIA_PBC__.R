...
##Select one of the two following lines, depending on which you are interested in. Either General or Specific.

# Here we put in the separate PBC and MAIA variable, all separate measures. This is the specific condition.
columns_of_interest <- c("PBC_1","PBC_2","PBC_3","PBC_4","PBC_5","MAIA_1_1","MAIA_1_2","MAIA_1_3","MAIA_1_4","MAIA_1_5","MAIA_1_6","MAIA_1_7","MAIA_1_8","MAIA_1_9","MAIA_1_10","MAIA_1_11","MAIA_1_12","MAIA_1_13","MAIA_1_14","MAIA_1_15","MAIA_2_1","MAIA_2_1","MAIA_2_2","MAIA_2_3","MAIA_2_4","MAIA_2_5","MAIA_2_6","MAIA_2_7","MAIA_2_8","MAIA_2_9","MAIA_2_10","MAIA_2_11","MAIA_2_12","MAIA_2_13","MAIA_2_14","MAIA_2_15","MAIA_2_16")

# Here we put the combined PBC and combined MAIA measures. This is the general condition. 
...
##Here we check for significance with t-tests and Benjamini-Hochberg correction

# Number of pairwise comparisons
num_comparisons <- nrow(edge_list)

# Perform pairwise t-tests between all variables
results <- sapply(1:nrow(edge_list), function(i) {
  from_var <- edge_list$from[i]
  to_var <- edge_list$to[i]
  t_result <- t.test(scores[, from_var], scores[, to_var])
  c(from_variable = from_var,
    to_variable = to_var,
...
results_df$is_significant <- results_df$adjusted_p_value < 0.05

# Print all comparisons with significance indicated
print(results_df)
